...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    prompt = f"""Ты персональный коуч по энергии и благополучию. Проанализируй дневник пользователя за последние недели.

ДАННЫЕ:
{entries_text}

ТВОЯ ЗАДАЧА:
1. Найди паттерны: что повышает энергию пользователя, что снижает
2. Определи главные факторы влияния (сон, работа, социальные контакты, отдых)
3. Обрати внимание на эмоциональный фон в записях

ФОРМАТ ОТВЕТА:
Анализ (3-4 предложения): 
- Общая динамика энергии
- Ключевые паттерны и триггеры
- Эмоциональное состояние

Рекомендации (4-6 конкретных действий):
- Что продолжать делать (что работает)
- Что изменить (что отнимает энергию)
- Новые привычки для стабильности энергии

Будь конкретным, персональным и поддерживающим. Опирайся только на данные из записей."""
